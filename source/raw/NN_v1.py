...
prout = 1       #numero de predicciones del modelo
train_df = df[0:int(n*0.7)]
val_df = df[int(n*0.7):int(n*0.9)]
test_df = df[int(n*0.9):]

x_train0 = np.array(train_df[0:len(train_df)-prout],dtype=float)
x_train = np.random.rand(len(train_df)-spin,spin,9)
for i in range(0,len(train_df)-spin-1):
    for j in range(0,spin-1):
        for k in range(0,8):
            x_train[i,j,k] = x_train0[i+j,k]
y_train = np.array(train_df[spin:len(train_df)].drop(['Hora','Media Red','Media Green','Media Blue'], axis=1),dtype=float)
#y_train = np.reshape(y_train, [len(train_df)-spin, 1, 5])

x_val0 = np.array(val_df[0:len(val_df)-prout],dtype=float)
x_val = np.random.rand(len(val_df)-spin,spin,9)
for i in range(0,len(val_df)-spin-1):
    for j in range(0,spin-1):
        for k in range(0,8):
            x_val[i,j,k] = x_val0[i+j,k]
y_val = np.array(val_df[spin:len(val_df)].drop(['Hora','Media Red','Media Green','Media Blue'], axis=1),dtype=float)
#y_val = np.reshape(y_val, [len(val_df)-spin, 1, 5])

x_test0 = np.array(test_df[0:len(test_df)-prout],dtype=float)
x_test = np.random.rand(len(test_df)-spin,spin,9)
for i in range(0,len(test_df)-spin-1):
    for j in range(0,spin-1):
        for k in range(0,8):
            x_test[i,j,k] = x_test0[i+j,k]
y_test = np.array(test_df[spin:len(test_df)].drop(['Hora','Media Red','Media Green','Media Blue'], axis=1),dtype=float)
...
#Modelo LSTM
model = keras.models.Sequential()
model.add(keras.layers.LSTM(units=200, activation = 'relu', input_shape=[4,9]))
...
X_new = np.reshape(x_test[0], [1,spin,9])
